...
describe('messageToActivity', () => {
  it('converts team_status started to harness_decision', () => {
    const msg: WSMessage = { type: 'team_status', status: 'started' }
  ...
  it('converts team_status failed to error', () => {
    const msg: WSMessage = { type: 'team_status', status: 'failed', error: '超时' }
  ...
  it('converts team_status failed with no error message', () => {
    const msg: WSMessage = { type: 'team_status', status: 'failed' }
    const entry = messageToActivity(msg, 1)
    expect(entry!.content).toContain('处理失败')
  })

  it('returns null for unknown team_status', () => {
    const msg: WSMessage = { type: 'team_status', status: 'unknown_status' }
...
describe('deriveTeamState', () => {
  ...
  it('activates team-lead on team start', () => {
  ...
  it('handles failed team', () => {
    const messages: WSMessage[] = [
      { type: 'team_status', status: 'started' },
      { type: 'team_status', status: 'failed', error: 'timeout' },
    ]
    const state = deriveTeamState(messages)
